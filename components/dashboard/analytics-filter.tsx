...
import { addDays, formatLocalizedDate } from "@/lib/date";
      ...
      <section className="space-y-2">
        ...
        <p className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          {draft.allTime
            ...
            : draft.range.from && draft.range.to
              ? `${formatLocalizedDate(draft.range.from)} - ${formatLocalizedDate(draft.range.to)}`
              : draft.range.from
                ? `${formatLocalizedDate(draft.range.from)} - select end date`
